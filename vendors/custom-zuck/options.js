...
export const optionsDefault = (option) => ({
    ...
    template: {
        ...
        <div class="story ${itemData['seen'] === true ? 'seen' : ''}">
          <a class="item-link" ${itemData['link'] ? `href="${itemData['link'] || ''}"` : ''}>
            <span class="item-preview">
              <img lazy="eager" src="${option('avatars') || !itemData['currentPreview']
                ? itemData['photo']
                : itemData['currentPreview']}" />
            </span>
            <span class="info" itemProp="author" itemScope itemType="http://schema.org/Person">
              <strong class="name" itemProp="name">${itemData['name']}</strong>
              <span class="time">${timeAgo(itemData['lastUpdated'] || itemData['time'], option('language')) || ''}</span>
            </span>
          </a>
                ...
                <div class="head">
                  <div class="left">
                    ...
                    <div class="info">
                      ...
                      <span class="time">${timeAgo(storyData['time'], option('language')) || ''}</span>
                    </div>
                  </div>
        ...
        viewerItemPointerProgress(style) {
        ...
        viewerItemPointer(index, currentIndex, item) {
            return `<span
                ...
                data-index="${index}" data-item-id="${item['id']}">
                  ${option('template')['viewerItemPointerProgress'](`animation-duration:${safeNum(item['length']) ? item['length'] : '3'}s`)}
              </span>`;
        },
        viewerItemBody(index, currentIndex, item) {
            return `<div
                ...
                ${item['type'] === 'video'
                ? `<video class="media" data-length="${item.length}" ${item.loop ? 'loop' : ''} muted webkit-playsinline playsinline preload="auto" src="${item['src']}" ${item['type']}></video>
                    <b class="tip muted">${option('language')['unmute']}</b>`
                : `<img loading="auto" class="media" src="${item['src']}" ${item['type']} />
                `}

                ${item['link']
                ? `<a class="tip link" href="${item['link']}" rel="noopener" target="_blank">
...
export const option = (options, _name) => {
    const self = (name) => {
